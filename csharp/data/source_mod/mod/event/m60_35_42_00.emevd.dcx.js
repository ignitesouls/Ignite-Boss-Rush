...
$Event(0, Default, function() {
    $InitializeEvent(0, 1035422150);
    RegisterBonfire(1035420000, 1035421950, 0, 0, 0, 5);
    if (EventFlag(1049308066)) { //if boss selected (omenkiller)
        ...
        $InitializeCommonEvent(0, 90005780, 1035420800, 1035422160, 1035422161, 1035420710, SummonSignType.NPCWhiteSign, 1035422700, 0, true, 0);
    ...
    $InitializeEvent(0, 1035420712);
    $InitializeEvent(0, 1035422151);
});
...
$Event(50, Default, function() {
    ...
    $InitializeCommonEvent(0, 90005260, 1035420800, 1035422800, 42, 0, -1);
    $InitializeCommonEvent(0, 90005250, 1035420201, 1035422204, 0, -1);
    $InitializeCommonEvent(0, 90005251, 1035420202, 8, 0, -1);
    $InitializeCommonEvent(0, 90005251, 1035420203, 8, 0, -1);
    $InitializeCommonEvent(0, 90005250, 1035420204, 1035422204, 0, -1);
    $InitializeCommonEvent(0, 90005201, 1035420205, 30004, 20004, 8, 0, 0, 0, 0, 0);
    $InitializeCommonEvent(0, 90005250, 1035420206, 1035422204, 0, -1);
    $InitializeCommonEvent(0, 90005250, 1035420207, 1035422204, 0, -1);
    $InitializeCommonEvent(0, 90005251, 1035420208, 5, 0, -1);
...
// 裸ラニ_カットシーン後消す処理 -- Naked Rani_Erasing after cutscene
...
// NPC219_足萎え村のNPC_NPC初期化イベント_初期状態 -- NPC219_NPC of Withered Village_NPC initialization event_Initial state
$Event(1035423700, Restart, function(chrEntityId, assetEntityId) {
...
L11:
    ...
    ForceAnimationPlayback(chrEntityId, 930022, false, false, false);
...
// NPC219_足萎え村のNPC_擬態解除 -- NPC219_NPC of the withered village_Mimicry canceled
$Event(1035420701, Restart, function(chrEntityId, assetEntityId) {
    ...
    if (!EventFlag(1035420701)) {
        ...
        ForceAnimationPlayback(chrEntityId, 20042, false, false, false);
        EnableCharacter(chrEntityId);
        WaitFixedTimeFrames(1);
...
// NPC219_足萎え村のNPC_死亡アニメ再生 -- NPC219_NPC of the withered village_Death animation playback
$Event(1035423702, Restart, function(chrEntityId) {
    WaitFixedTimeFrames(1);
    EndIf(!EventFlag(3540));
    EndIf(!EventFlag(3545));
    WaitFor(EventFlag(1035429210));
    DisableCharacterCollision(chrEntityId);
    ForceAnimationPlayback(chrEntityId, 20054, false, false, false);
...
// NPC219_足萎え村のNPC_敵対アニメ再生 -- NPC219_NPC of the withered village_hostile animation playback
$Event(1035423703, Restart, function(entityId) {
    WaitFixedTimeFrames(1);
    EndIf(!PlayerIsInOwnWorld());
    EndIf(!EventFlag(3545));
    EndIf(!EventFlag(3540));
    WaitFor(EventFlag(3541));
    ForceAnimationPlayback(entityId, 20052, false, false, false);
    RestartEvent();
});

// NPC106_ラニ_NPC初期化イベント_カットシーン直後 -- NPC106_Rani_NPC initialization event_Immediately after cutscene
$Event(1035420710, Restart, function(chrEntityId) {
    DisableNetworkSync();
    WaitFixedTimeFrames(1);
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(3740)) {
...
// NPC106_ラニ_カットシーン直後_会話後に消える -- NPC106_Rani_Immediately after cutscene_Disappears after conversation
$Event(1035420711, Restart, function(entityId) {
    EndIf(!PlayerIsInOwnWorld());
    EndIf(EventFlag(1034509406));
    WaitFor(EventFlag(1034509406));
    ForceAnimationPlayback(entityId, 20007, false, false, false);
    WaitFixedTimeSeconds(6);
    SetEventFlagID(1035429255, ON);
});

// NPC106_ラニ_カットシーン直後_現れる -- NPC106_Rani_Appears immediately after the cutscene
...
$Event(1035420720, Restart, function(chrEntityId, assetEntityId) {
    WaitFixedTimeFrames(1);
    DisableNetworkSync();
    if (PlayerIsInOwnWorld()) {
        if (!EventFlag(4220)) {
