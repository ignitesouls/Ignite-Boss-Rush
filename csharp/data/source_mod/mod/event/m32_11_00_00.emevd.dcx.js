...
$Event(0, Default, function() {
    ...
    $InitializeEvent(0, 32112800);
    $InitializeEvent(0, 32112810);
    $InitializeEvent(0, 32112811);
    $InitializeEvent(0, 32112849);
    ...
    $InitializeCommonEvent(0, 90005646, 32110800, 32112840, 32112841, 32111840, 32112840, 32, 11, 0, 0);
    $InitializeCommonEvent(0, 900005610, 32111680, 100, 800, 0);
});

// プリコンストラクタ -- preconstructor
$Event(50, Default, function() {
    $InitializeEvent(0, 32110519);
    $InitializeCommonEvent(0, 90005200, 32110200, 30007, 20007, 32112200, 0, 0, 0, 0, 0);
    $InitializeCommonEvent(0, 90005200, 32110201, 30007, 20007, 32112200, 0.5, 0, 0, 0, 0);
    ...
    $InitializeCommonEvent(0, 90005200, 32110207, 30007, 20007, 32112206, 0.5, 0, 0, 0, 0);
    ...
    $InitializeCommonEvent(0, 90005200, 32110210, 30007, 20007, 32112209, 0.2, 0, 0, 0, 0);
    $InitializeCommonEvent(0, 90005250, 32110210, 32112209, 0, -1);
    $InitializeCommonEvent(0, 90005201, 32110211, 30006, 20006, 5, 0, 0, 1, 0, 0);
...
$Event(32112800, Restart, function() {
    EndIf(!EventFlag(1049308166)); //end if boss not selected
    ...
    $InitializeCommonEvent(0, 90009810, 1049307566);
});
...
// 分身体リセット_XX -- Clone body reset_XX
$Event(32112820, Restart, function(chrEntityId, chrEntityId2) {
    WaitFor(CharacterHasSpEffect(chrEntityId, 16714));
    WarpCharacterAndCopyFloor(chrEntityId, TargetEntityType.Asset, chrEntityId2, 100, chrEntityId);
    WaitFixedTimeSeconds(1);
    ClearSpEffect(chrEntityId, 16714);
    RestartEvent();
});

// 分身体無効化_XX -- Clone body invalidation_XX
$Event(32112830, Restart, function(chrEntityId) {
    if (EventFlag(32110800)) {
...
L0:
    WaitFor(!CharacterHasSpEffect(chrEntityId, 5038) && CharacterDead(32110800));
    DisableCharacter(chrEntityId);
    DisableCharacterCollision(chrEntityId);
});
