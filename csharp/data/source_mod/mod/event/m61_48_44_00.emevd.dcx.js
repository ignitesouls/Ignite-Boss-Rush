...
$Event(0, Default, function() {
    ...
    $InitializeEvent(0, 2048442800);
    $InitializeEvent(0, 2048442810);
    $InitializeEvent(0, 2048442849);
    $InitializeEvent(0, 2048442840);
    $InitializeEvent(0, 2048442811);
    $InitializeCommonEvent(0, 90005780, 2048440800, 2048442160, 2048442161, 2048440720, SummonSignType.WhiteSign, 2048442721, 0, true, 0); //enable summon visibility
    ...
    $InitializeCommonEvent(0, 90005783, 2048440800, 2048442160, 2048442161, 2048440720, 2048442721, 0, 1); //areaentityID to 0 to prevent leda leaving
    ...
    $InitializeCommonEvent(0, 90005251, 2048440304, 25, 0, 0);
    $InitializeEvent(0, 2048442580);
    $InitializeCommonEvent(0, 90005501, 2048440510, 2048440511, 0, 2048441510, 2048441511, 2048441512, 2048440512);
    $InitializeEvent(0, 2048442510);
    $InitializeCommonEvent(0, 900005610, 2048441200, 100, 800, 0);
    $InitializeCommonEvent(0, 90005790, 0, 2048440180, 2048442181, 2048442182, 2048440750, 22, 2048442710, 2048442711, 0, 2045429290, false, 0);
    ...
    $InitializeCommonEvent(0, 90005759, 2045429280, 4385, 4960, 2048440740, 4950, 2045429290, 4950, 4956, 2045429281, 2045429282, 4901, 1, 2045429340, 30);
    ...
    $InitializeCommonEvent(0, 90005766, 2048442181, 2048440750, 100, 2045429277, 4960);
...
$Event(2048442200, Restart, function(chrEntityId) {
    if (!CharacterHasSpEffect(chrEntityId, 13690)) {
        SetSpEffect(chrEntityId, 13690);
    }
});

//rellana
$Event(2048442800, Default, function() {
    EndIf(!EventFlag(1049308168) && !EventFlag(1049309740) && !EventFlag(1049309740)); //end if boss not selected
    ...
    if (EventFlag(1049308168)) //full fight
        $InitializeCommonEvent(0, 90009810, 1049307568);
    else if (EventFlag(1049309740)) //phase 2 restore hp
        $InitializeCommonEvent(0, 90009810, 1049307740);
    else if (EventFlag(1049309741)) //phase 2 normal hp
        $InitializeCommonEvent(0, 90009810, 1049307741);
});
...
L2:
    ...
    WaitFixedTimeSeconds(0.5);
    if (EventFlag(1049309740) || EventFlag(1049309741)) //if phase 2 selected
        SetSpEffect(2048440800, 10493044);
...
$Event(2048442811, Restart, function() {
    ...
    if (EventFlag(1049309740)) //if phase 2 & restore hp
        SetSpEffect(2048440800, 10493050);
});
...
$Event(2048440715, Restart, function(eventFlagId, eventFlagId2, eventFlagId3) {
...
$Event(2048440716, Restart, function(eventFlagId, eventFlagId2, eventFlagId3) {
    WaitFixedTimeFrames(1);
    EndIf(!PlayerIsInOwnWorld());
    SetEventFlagID(eventFlagId, OFF);
    EndIf(EventFlag(eventFlagId2));
