...
$Event(0, Default, function() {
    ...
    if (EventFlag(1049308060)) { //if boss selected (bell bearing hunter)
...
$Event(50, Default, function() {
    ...
    $InitializeCommonEvent(0, 90005261, 1037460200, 1037462600, 3, 0, 3020);
    $InitializeCommonEvent(0, 90005261, 1037460201, 1037462600, 3, 0, 3020);
    $InitializeCommonEvent(0, 90005261, 1037460202, 1037462600, 3, 0, 3020);
    $InitializeCommonEvent(0, 90005261, 1037460203, 1037462600, 3, 0, 1700);
    $InitializeCommonEvent(0, 90005261, 1037460204, 1037462600, 3, 0, 1700);
});

// NPC330_贖罪司祭_NPC初期化イベント_初期状態 -- NPC330_Atonement Priest_NPC initialization event_Initial state
$Event(1037460700, Restart, function(chrEntityId) {
    DisableNetworkSync();
...
S0:
    ...
    ForceAnimationPlayback(chrEntityId, 930001, false, false, false);
    Goto(L20);
L3:
...
// NPC330贖罪司祭_殻にこもる -- NPC330 Atonement Priest_Remains in a shell
$Event(1037460702, Restart, function(chrEntityId) {
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(HasDamageType(chrEntityId, 0, DamageType.Unspecified));
    SetSpEffect(chrEntityId, 9750);
    RestartEvent();
});

// NPC330_贖罪司祭_免罪対象判定_汎用 -- NPC330_Atonement Priest_Absolution target determination_General purpose
$Event(1037460710, Restart, function(eventFlagId) {
    EndIf(!PlayerIsInOwnWorld());
...
// NPC330_贖罪司祭_免罪対象判定_特殊 -- NPC330_Atonement Priest_Absolution target determination_Special
$Event(1037460711, Restart, function(eventFlagId) {
    EndIf(!PlayerIsInOwnWorld());
    SetEventFlagID(eventFlagId, OFF);
...
// NPC330_贖罪司祭_免罪実行 -- NPC330_Atonement Priest_Performing absolution
$Event(1037460719, Restart, function(eventFlagId) {
    EndIf(!PlayerIsInOwnWorld());
    SetEventFlagID(eventFlagId, OFF);
