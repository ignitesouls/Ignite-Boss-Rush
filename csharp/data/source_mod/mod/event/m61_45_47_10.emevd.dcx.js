...
$Event(0, Default, function() {
    ...
    $InitializeCommonEvent(0, 90005211, 2045470221, 30000, 20001, 2045472220, 3, 0, 0, 0, 0, 0);
    $InitializeCommonEvent(0, 90005261, 2045470220, 2045472220, 1, 0, 3010);
    ...
    $InitializeEvent(0, 2045472690);
    $InitializeEvent(0, 2045472695);
    $InitializeEvent(0, 2045472699);
    $InitializeEvent(0, 2045472540, 2045470540, 2045472540);
    //devonia events
    $InitializeEvent(0, 2045479995);
    $InitializeEvent(0, 2045479996);
    $InitializeEvent(0, 2045479997);
});

//devonia battle
$Event(2045479995, Default, function() {
    WaitFor(HPRatio(2045470200) <= 0.55); //trigger for bgm heatup
    SetEventFlagID(2045472990, ON);
    EndIf(!EventFlag(1049308208)); //end if boss not selected
    ...
    $InitializeCommonEvent(0, 90009810, 1049307608);
});

//devonia bgm and hp bar
$Event(2045479996, Default, function() {
    EndIf(CharacterDead(2045470200));
    DisableCharacterHPBarDisplay(2045470200);
    WaitFor(CharacterAIState(2045470200, AIStateType.Combat));
    DisplayBossHealthBar(Enabled, 2045470200, 0, 910000000);
    SetBossBGM(920200, BossBGMState.Start);
    if (EventFlag(2045472990)) //if hp already under 55%, switch to heatup bgm immediately
        SetBossBGM(920200, BossBGMState.HeatUp);
    WaitFor(!CharacterAIState(2045470200, AIStateType.Combat) || CharacterDead(2045470200));
    DisplayBossHealthBar(Disabled, 2045470200, 0, 910000000);
    SetBossBGM(920200, BossBGMState.Stop1);
    RestartEvent();
});

//devonia bgm heatup
$Event(2045479997, Default, function() {
    WaitFor(EventFlag(2045472990) || CharacterDead(2045470200));
    if (CharacterDead(2045470200)) //if already dead
        EndEvent();
    else if (EventFlag(2045472990)) //if hp under 55%
        SetBossBGM(920200, BossBGMState.HeatUp);
});

$Event(2045472540, Restart, function(eventFlagId, areaEntityId) {
...
$Event(2045472542, Restart, function(eventFlagId, assetEntityId, assetEntityId2, objactEventFlag, objactParamId, animationId, animationId2, objactEventFlag2, objactParamId2) {
